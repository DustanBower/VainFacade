...
namespace VainFacadePlaytest.Blitz
{
    public class UnnecessaryRoughnessCardController : PlaybookCardController
    {
        ...
        private IEnumerator DestroyCardsToChargeResponse(PhaseChangeAction pca)
        {
            ...
            IEnumerator destroyHeroCoroutine = base.GameController.SelectAndDestroyCards(DecisionMaker, new LinqCardCriteria((Card c) => c.IsHero && (c.IsOngoing || IsEquipment(c)), "hero Ongoing or Equipment"), H - 2, requiredDecisions: H - 2, storedResultsAction: results, responsibleCard: base.Card, cardSource: GetCardSource());
            if (base.UseUnityCoroutines)
            {
