...
namespace VainFacadePlaytest.Grandfather
{
    public class ArrowOfTimeCardController : CardController
    {
        ...
        public override void AddTriggers()
        {
            ...
            AddTrigger((DiscardCardAction dca) => dca.WasCardDiscarded && dca.CardSource != null && dca.CardSource.Card != null && IsVillain(dca.CardSource.Card) && dca.Origin.IsDeck && dca.Origin.IsHero && dca.Origin.OwnerTurnTaker != null && FindInHeroOrder(dca.Origin.OwnerTurnTaker) != -1 && !IsPropertyTrue(GeneratePerTargetKey(FindPerTurnTakerKey(dca.Origin.OwnerTurnTaker), dca.CardSource.Card)), DestroyOrAddTokenResponse, new TriggerType[] { TriggerType.DestroyCard, TriggerType.AddTokensToPool }, TriggerTiming.After);
            AddTrigger((MoveCardAction mca) => mca.WasCardMoved && mca.CardSource != null && mca.CardSource.Card != null && IsVillain(mca.CardSource.Card) && mca.Origin.IsDeck && mca.Origin.IsHero && mca.Destination.IsTrash && mca.IsDiscard && mca.Origin.OwnerTurnTaker != null && FindInHeroOrder(mca.Origin.OwnerTurnTaker) != -1 && !IsPropertyTrue(GeneratePerTargetKey(FindPerTurnTakerKey(mca.Origin.OwnerTurnTaker), mca.CardSource.Card)), DestroyOrAddTokenResponse, new TriggerType[] { TriggerType.DestroyCard, TriggerType.AddTokensToPool }, TriggerTiming.After);
