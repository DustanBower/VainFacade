...
namespace VainFacadePlaytest.Grandfather
{
    public class CountingDownCardController : GrandfatherUtilityCardController
    {
            ...
            : base(card, turnTakerController)
        {
            ...
            SpecialStringMaker.ShowNumberOfCardsInPlay(new LinqCardCriteria((Card c) => c.IsTarget && !IsHeroTarget(c) && !c.DoKeywordsContain(CovertKeyword), "non-Covert non-hero", false, false, "target", "targets"));
        ...
        public override void AddTriggers()
        {
            ...
            AddEndOfTurnTrigger((TurnTaker tt) => tt == base.TurnTaker && FindCardsWhere(new LinqCardCriteria((Card c) => c.IsInPlayAndHasGameText && c.IsTarget && !IsHeroTarget(c) && !c.DoKeywordsContain(CovertKeyword), "non-Covert non-hero", false, false, "target", "targets"), visibleToCard: GetCardSource()).Count() < 2, (PhaseChangeAction pca) => base.GameController.PlayTopCard(DecisionMaker, FindEnvironment(), responsibleTurnTaker: base.TurnTaker, showMessage: true, cardSource: GetCardSource()), TriggerType.PlayCard);
        ...
        private IEnumerator SecondHighestDealsDamageResponse(PhaseChangeAction pca)
        {
            ...
            IEnumerator findCoroutine = base.GameController.FindTargetWithHighestHitPoints(2, (Card c) => c.IsTarget && !IsHeroTarget(c), results, cardSource: GetCardSource());
            ...
            if (attacker != null)
            {
                IEnumerator damageCoroutine = DealDamageToHighestHP(attacker, 1, (Card c) => IsHeroTarget(c), (Card c) => H, DamageType.Melee, numberOfTargets: () => 2);
                if (base.UseUnityCoroutines)
                {
