...
namespace VainFacadePlaytest.Carnaval
{
    public class ALittleExtraCardController : CardController
    {
        ...
        private IEnumerator UseGrantedPower()
        {
            ...
            IEnumerator destroyCoroutine = base.GameController.SelectAndDestroyCard(DecisionMaker, new LinqCardCriteria((Card c) => c.IsHero && (c.IsOngoing || IsEquipment(c)), "hero Ongoing or Equipment"), false, responsibleCard: base.Card, cardSource: GetCardSource());
            if (base.UseUnityCoroutines)
            {
