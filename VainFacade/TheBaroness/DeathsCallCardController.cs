...
namespace VainFacadePlaytest.TheBaroness
{
    public class DeathsCallCardController : BaronessUtilityCardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            List<TurnTaker> heroes = base.GameController.FindTurnTakersWhere((TurnTaker tt) => IsHero(tt) && !tt.IsIncapacitatedOrOutOfGame).ToList();
            ...
            IEnumerator damageCoroutine = DealDamage(base.CharacterCard, (Card c) => IsHeroTarget(c) && DestroyedCount(c.Owner, nonBloodDestroyed) <= 0, H - 1, DamageType.Infernal);
            if (base.UseUnityCoroutines)
            {
