...
namespace VainFacadePlaytest.Node
{
    public class PsychologicalFortitudeCardController : NodeUtilityCardController
    {
            ...
            : base(card, turnTakerController)
        {
            ...
            SpecialStringMaker.ShowListOfCardsInPlay(new LinqCardCriteria((Card c) => IsConnected(c) && IsHero(c) && (IsOngoing(c) || IsEquipment(c)), "Connected hero Ongoing or Equipment"));
        }
        ...
        public override void AddTriggers()
        {
            ...
            AddTrigger((DestroyCardAction dca) => dca.CardToDestroy != null && dca.CardToDestroy.Card != null && IsConnected(dca.CardToDestroy.Card) && IsHero(dca.CardToDestroy.Card) && (IsOngoing(dca.CardToDestroy.Card) || IsEquipment(dca.CardToDestroy.Card)) && dca.ResponsibleCard != dca.CardToDestroy.Card && dca.ResponsibleCard != base.Card, DestroyToProtectResponse, new TriggerType[] { TriggerType.DestroyCard, TriggerType.CreateStatusEffect}, TriggerTiming.Before);
        }
        ...
        private IEnumerator DestroyToProtectResponse(DestroyCardAction dca)
        {
            ...
            IEnumerator destroyCoroutine = base.GameController.SelectAndDestroyCard(DecisionMaker, new LinqCardCriteria((Card c) => c != dca.CardToDestroy.Card && IsConnected(c) && IsHero(c) && (IsOngoing(c) || IsEquipment(c)), "other Connected hero Ongoing or Equipment"), true, destruction, responsibleCard: base.Card, cardSource: GetCardSource());
